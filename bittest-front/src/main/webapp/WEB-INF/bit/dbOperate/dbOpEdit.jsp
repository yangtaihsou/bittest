...
<form id="erpform" onsubmit="return false;" class="form-horizontal form-bordered" role="form">
    <input type="hidden" name="interfaceId" value="${dbOp.interfaceId}"/>
    <div class="box-body">
        <div class="form-group">
            <label class="from-control col-xs-4">名称</label>
            <input class="form-control" columWidthClass="col-xs-8" type="text" value="${dbOp.name}" name="name" id="name" required/>
        </div>
        <div class="form-group">
            ...
            <select class="form-control" columWidthClass="col-xs-8" id="url" name="url" required >
                <c:forEach var="db" items="${db}" >
                    ...
                    <c:if test="${db.databaseId==dbOp.url}">
                        selected="selected"
                    </c:if>
                    >${db.name}</option>
                </c:forEach>
        ...
        <div class="form-group">
            ...
            <select class="form-control" columWidthClass="col-xs-8" id="method" name="method" required >
                <option value="insert" <c:if test="${dbOp.method eq 'insert'}">
                    selected="selected"
                </c:if>>insert新增</option>
                <option value="update" <c:if test="${dbOp.method eq 'update'}">
                    selected="selected"
                </c:if>>修改or删除</option>
            </select>
        </div>
        <div class="form-group">
            ...
            <textarea id="body" name="body" class="form-control" rows="4" required columWidthClass="col-xs-8">${dbOp.body}</textarea>
        </div>
        <div class="form-group">
            <label class="from-control col-xs-4">备&nbsp;&nbsp;&nbsp;&nbsp;注</label>
            <textarea id="remark" name="remark" class="form-control" rows="4" columWidthClass="col-xs-8">${dbOp.remark}</textarea>
        </div>
    </div><!-- /.box-body -->
...
<footer>
    ...
    <script type="text/javascript">
        ...
        function dataSubmit(){
            ...
            me.postAjaxCallBack(erpform,'/dbOp/dbOpEdit',submitCallBack,"");
        }
