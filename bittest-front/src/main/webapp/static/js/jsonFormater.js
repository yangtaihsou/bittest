function JsonFormater(opt) {
    this.options = $.extend({
        ...
        singleTab: "  ",
        quoteKeys: true,
        imgCollapsed: "/static/css/i/Collapsed.gif",
        imgExpanded: "/static/css/i/Expanded.gif",
        isCollapsible: true
    }, opt || {});
...
JsonFormater.prototype = {
    ...
    doFormat: function (json) {
        ...
        try {
         ...
         //   $(this.options.dom).html("<pre class='jf-CodeContainer' style='color:red'> JSON数据格式不正确:" + json + "</pre>");
            $(this.options.dom).html("<pre class='jf-CodeContainer' style='color:#007777;font-weight:bold;font-size:15'>" + json + "</pre>");
            this.isFormated = false;
        }
    },
    bindEvent: function () {
