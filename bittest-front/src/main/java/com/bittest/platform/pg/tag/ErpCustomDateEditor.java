 ...
 * @author yechao1
 */
public class ErpCustomDateEditor extends PropertyEditorSupport {

    public static String FORMART_DATETIME = "yyyy-MM-dd HH:mm:ss";

    public static String FORMART_DATE = "yyyy-MM-dd";

    public static String FORMART_MONTH = "yyyy-MM";
    ...
    public static Pattern REGPATTERN_MONTH = Pattern.compile("\\d{4}-\\d{1,2}");
    ...
    public ErpCustomDateEditor(DateFormat dateFormat, boolean allowEmpty) {
        this.dateFormat = dateFormat;
        this.allowEmpty = allowEmpty;
        this.exactDateLength = -1;
    }

    public ErpCustomDateEditor(DateFormat dateFormat, boolean allowEmpty, int exactDateLength) {
        this.dateFormat = dateFormat;
        this.allowEmpty = allowEmpty;
            ...
            try {
                if (dateFormat == null) {
                    if (REGPATTERN_DATE.matcher(text).matches()) {
                        ...
                        sdf.setLenient(false);
                        setValue(sdf.parse(text));
                    } else if (REGPATTERN_MONTH.matcher(text).matches()) {
                        SimpleDateFormat sdf = new SimpleDateFormat(FORMART_MONTH);
                        sdf.setLenient(false);
                        setValue(sdf.parse(text));
                    }
                } else {
